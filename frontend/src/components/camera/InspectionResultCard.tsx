...
export function InspectionResultCard({ data, onDismiss, onViewDetails }: InspectionResultCardProps) {
  ...
  return (
    <Card className="m-4 bg-[#111927]/95 backdrop-blur-md shadow-2xl border border-[#23334d] space-y-4">
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-3">
          <div className={`p-2.5 rounded-xl text-white ${data.needs_escalation ? 'bg-rose-500 shadow-md shadow-rose-500/20' : 'bg-emerald-500'}`}>
            {data.needs_escalation ? <AlertTriangle className="w-6 h-6" /> : <CheckCircle className="w-6 h-6" />}
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-bold text-white text-base">{data.machine_id} Inspection</h3>
              <Badge variant={isHighConfidence ? 'success' : isLowConfidence ? 'accent' : 'warning'}>
                {formatConfidence(data.confidence)} Confidence
              </Badge>
            </div>
            <p className="text-xs text-slate-300 mt-0.5">Finding: {data.finding}</p>
          </div>
        </div>

        <button onClick={onDismiss} className="text-slate-400 hover:text-white font-bold text-sm">
          ✕
        </button>
      </div>

      {data.needs_escalation && (
        <div className="p-3 bg-rose-950/40 border border-rose-800/60 rounded-xl flex items-center gap-2 text-xs font-semibold text-rose-300">
      ...
      {data.repair_steps && data.repair_steps.length > 0 && (
        <div className="space-y-1.5 border-t border-[#23334d] pt-3">
          <div className="flex items-center gap-1.5 text-xs font-bold text-slate-200 uppercase tracking-wider">
            <Wrench className="w-3.5 h-3.5 text-sky-400" /> Recommended Repair Steps
          </div>
          <ul className="space-y-1 text-xs text-slate-200 pl-4 list-disc font-normal">
      ...
      <div className="flex items-center justify-end gap-2 border-t border-[#23334d] pt-3">
        {onViewDetails && (
